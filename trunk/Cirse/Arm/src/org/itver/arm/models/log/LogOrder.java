...
public final class LogOrder extends Model{
    ...
    public void addStep(LogStep step){
        //Si el LogStep ya esta o es null, no hace nada
        if(this.steps.contains(step) ||
           step == null)
            return;
        //Si ya hay un LogStep con la articulacion dada, actualiza sus valores
        ...
        //Agrega el step en caso contrario
        else
            this.steps.add(step);
...
//        if(time <= 0)
        long time = 0;
            for(LogStep step : this.steps)
                if(time < step.getTime())
                    time = step.getTime();
        return time;
    }
